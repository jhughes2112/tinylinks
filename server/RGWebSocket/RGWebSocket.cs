...
namespace ReachableGames
{
	namespace RGWebSocket
	{
		...
		public class RGWebSocket
		{
			...
			public RGWebSocket(HttpListenerContext? httpContext, Func<RGWebSocket, string, Task> onReceiveMsgText, Func<RGWebSocket, PooledArray, Task> onReceiveMsgBinary, Func<RGWebSocket, Task> onDisconnectionCb, ILogging logger, string displayId, WebSocket webSocket)
			{
			...
			public void Send(PooledArray binMsg)
			{
				try
				{
					...
					if (_cancellationTokenSource!=null && _cancellationTokenSource.IsCancellationRequested==false)
					{
						...
						QueuedSendMsg qsm = new QueuedSendMsg() { binMsg = binMsg, enqueuedTick = DateTime.UtcNow.Ticks };
...
//					_outgoing.Add(qsm);        // this automatically locks the queue on add/remove
//					_releaseSendThread.Set();  // unlock the send thread since there's work for it to do
					_logger.Log(EVerbosity.Error, $"RGWS - Send text is not supported!");
				}
				catch (Exception ex)
				{
			...
			private async Task Recv(CancellationToken token)
			{
				...
				_logger.Log(EVerbosity.Error, $"RGWS.Recv exiting {_displayId}");
			}
...
//										}
//										else  // text messages are not supported
										{
...
#endif
					_logger.Log(EVerbosity.Error, $"RGWS.Send exiting {_displayId}");
