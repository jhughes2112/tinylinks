...
namespace ReachableGames
{
	namespace RGWebSocket
	{
		...
		public class UnityWebSocket
		{
			...
			private LockingList<PooledArray>   _incomingMessages = new LockingList<PooledArray>();
			...
			private void Log(Logging.EVerbosity type, string message)
			{
				_logger.Log(type, $"{_loggerPrefix} {message}");
			}
			...
			private async Task DoConnection()
			{
				...
				try
				{
					...
					using (CancellationTokenSource connectTimeout = new CancellationTokenSource(_connectTimeoutMS))
					{
						...
						Log(Logging.EVerbosity.Warning, $"UWS Connected to {uri} http part");
					}

					_status = Status.Connected;
					_rgws = new RGWebSocket(null, OnReceiveText, OnReceiveBinary, OnDisconnect, _logger, uri.ToString(), wsClient);
					Log(Logging.EVerbosity.Warning, $"UWS Connected to {uri} rgws part");
				}
				catch (AggregateException age)
				{
					if (age.InnerException is OperationCanceledException)
					{
						...
						Log(Logging.EVerbosity.Error, _lastErrorMsg);
					}
			...
			public void Close()
			{
				...
				{
					_rgws!.Close();
					Log(Logging.EVerbosity.Warning, "UWS Closed.");
				}
			}
			...
			public void Shutdown()
			{
				if (_rgws!=null)
				{
					...
					_rgws = null;
					rgws.Shutdown().Wait();
					Log(Logging.EVerbosity.Warning, "UWS connection reset.");
				}
			...
			public bool Send(string msg)
			{
				if (_status == Status.Connected)
				{
					_rgws!.Send(msg);
#if RGWS_LOGGING
					Log(Logging.EVerbosity.Debug, $"UWS Sent {msg.Length} bytes");
#endif
					return true;
				}
				else
				{
					Log(Logging.EVerbosity.Debug, $"UWS Send called but status is {_status}");
				}
				return false;
			}
			...
			public void ReceiveAll(List<PooledArray> messageList)
			{
			...
			{
				Log(Logging.EVerbosity.Error, $"UWS Recv text is not supported!");
...
#if RGWS_LOGGING
				Log(Logging.EVerbosity.Debug, $"UWS Recv {msg.Length} bytes binary.  IncomingMessages={_incomingMessages.Count}");
#endif
				return Task.CompletedTask;
			}
			...
			{
				Log(Logging.EVerbosity.Warning, "UWS Disconnected.");
				_status = Status.ReadyToConnect;
