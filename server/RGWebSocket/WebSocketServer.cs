		...
		{		
			...
			public async Task StopListening()
			{
				if (_listener.IsListening)
				{
					...
					_listener.Close();  // this supposedly does not abort the websockets.
			...
			private async Task ListenerUpdate(int numListenerTasks)
			{
				...
				try
				{
					...
					for (int i=0; i<numListenerTasks; i++)
					{
						...
						listenerTasks.Add(t);
						_logger.Log(EVerbosity.Debug, "WebSocketServer.ListenerUpdate - adding listener");
					}
					...
					while (_listener.IsListening)  // loop forever until canceled
					{
						using (Task t = await Task.WhenAny(listenerTasks).ConfigureAwait(false))
						{
							...
							{
								_logger.Log(EVerbosity.Debug, $"WebSocketServer.ListenerUpdate - listener handled {connectTask.Status}");
				...
				{
					_logger.Log(EVerbosity.Debug, "WebSocketServer.ListenerUpdate - disposing listener tasks");
					...
					_logger.Log(EVerbosity.Debug, "WebSocketServer.ListenerUpdate - listener tasks dead");
				}
			}
